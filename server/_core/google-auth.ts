...
export function registerGoogleAuthRoutes(app: Express) {
    // Server-side auth callback handler (for backward compatibility)
    // This sets the cookie server-side and redirects appropriately
    app.get("/auth/callback", async (req: Request, res: Response) => {
        const sessionToken = req.query.sessionToken as string;
        
        if (!sessionToken) {
            console.error('[Auth Callback] No session token provided');
            res.redirect('/login?error=no_session_token');
            return;
        }
        ...
        try {
            ...
            if (!session) {
                console.error('[Auth Callback] Invalid session token');
                res.redirect('/login?error=invalid_session');
                return;
            }

            // Set the cookie server-side with proper options
            const cookieOptions = getSessionCookieOptions(req);
            res.cookie(COOKIE_NAME, sessionToken, { ...cookieOptions, maxAge: ONE_YEAR_MS });

            // Get user to check onboarding status
            let user;
            if (session.appId === "google-oauth") {
                user = await db.getUserByGoogleId(session.openId);
            } else if (session.openId.startsWith("email_")) {
                const userId = parseInt(session.openId.replace("email_", ""), 10);
                if (!isNaN(userId)) {
    ...
    app.get("/api/oauth/google/callback", async (req: Request, res: Response) => {
        ...
        try {
            ...
            // Set session cookie server-side (httpOnly, secure, etc.)
            const cookieOptions = getSessionCookieOptions(req);
            res.cookie(COOKIE_NAME, sessionToken, { ...cookieOptions, maxAge: ONE_YEAR_MS });

            console.log('[Google Auth] Cookie set, redirecting to dashboard');
