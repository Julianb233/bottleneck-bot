...
import './metallic-business-card.css';
...
export function MetallicBusinessCard({
  name = 'Rahil Vahora',
  ...
  const titleId = useMemo(
    () => `card-title-${Math.random().toString(36).slice(2, 8)}`,
    []
  );
  ...
  return (
    ...
    >
      ...
      >
        {/* content layer */}
        <div
          className={`content ${compact ? 'compact' : ''}`}
          style={{ 
            alignItems: alignCss, 
            textAlign,
            justifyContent: alignCss === 'center' ? 'center' : alignCss === 'right' ? 'flex-end' : 'flex-start'
          }}
        >
          {logoSrc ? (
            <img
          ...
          <div className="text-block" style={{ textAlign }}>
          ...
          {(email || phone || website) && (
            <div className="contact" style={{ textAlign }}>
              {email ? (
